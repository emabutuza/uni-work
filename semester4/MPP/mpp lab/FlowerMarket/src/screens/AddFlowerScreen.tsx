...
import { AdminAddFlowerScreenProps } from '../navigation/types';
...
export default function AddFlowerScreen({ navigation }: AdminAddFlowerScreenProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      ...
      formData.imageUrl.uri.trim() === ''
    ) {
      newErrors.imageUrl = 'Image URL is required';
    } else if (!isValidUrl(formData.imageUrl.uri)) {
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      try {
        const newFlower = FlowerService.createFlower(formData);
        Alert.alert(
          'Success',
        ...
        Alert.alert('Error', 'Failed to add flower. Please try again.');
      } finally {
      ...
      <ScrollView style={styles.container}>
        <View style={styles.formContainer}>
          ...
          <TextInput
            ...
            onChangeText={(text) => handleChange('imageUrl', { uri: text })}
            style={styles.input}
